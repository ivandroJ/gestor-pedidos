<form action="/solicitantes" method="POST" onsubmit="return confirm('Tem certeza?')">
    @csrf
    <div id="modal" class="fixed inset-0 bg-black bg-opacity-50 {{ old('nome') ? '' : 'hidden' }} flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-lg w-11/12 md:w-1/3 lg:w-1/2 p-6">
            ...
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-bold">Novo Solicitante</h2>
            ...
            <div class="mb-4">
                <x-input-text id="nome" name="nome" type="text" title="Nome"
                    placeholder="Insira o nome do Solicitante" value="{{ old('nome') }}"></x-input-text>
            </div>

             <div class="mb-4">
                <x-input-text id="email" name="email" type="email" title="E-Mail"
                    placeholder="Insira o nome do E-Mail" value="{{ old('email') }}"></x-input-text>
            </div>
